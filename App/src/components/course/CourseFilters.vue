<script setup>
// Presentational filter bar for the courses catalog. Two-way bound to the
// container's filter state -- same defineModel pattern as
// components/service/ServiceFilters.vue and components/catalog/ProductFilters.vue.
// No availability-type equivalent: courses don't have one (see
// CourseCardResource -- backend/app/Http/Resources/CourseCardResource.php).
...
// [DEFECT 1 fix] Price range and sort collapse behind a "Filtros" toggle --
// see useFilterCollapse.js's header comment and ProductFilters.vue's
// identical pattern. Courses have no domain dropdown, so this panel is the
// smallest of the three.
const { isOpen, toggle, contentId } = useFilterCollapse('course-filters')
...
// sort counts only if it deviates from "newest" (Más recientes, the default).
...
<template>
  <!-- Sticky, offset below the AppShell's own sticky top bar and kept at
       z-20 -- aligned with ProductFilters.vue/ServiceFilters.vue's
       convention (the two fixed nav bars own z-40; nothing in this
       screen's scope may reach that). Previously this section had no
       sticky positioning at all, unlike its two siblings -- now matched. -->
  <section class="bg-surface border-y border-blush-canvas/20 sticky top-[calc(var(--app-topbar-h)+var(--safe-area-top))] z-20">
    <div class="max-w-container-max mx-auto px-gutter py-6 flex flex-col gap-4">
      ...
      <!-- Collapsible: price range + sort. See ProductFilters.vue's
           identical comment on the grid-template-rows animation choice,
           prefers-reduced-motion handling, and :inert. -->
      <div
        :id="contentId"
        data-filters-panel
